...
interface ReadingStatsState {
  ...
  getAnnualReport: (userId: string, year: number) => Promise<AnnualReportData>;
...
export const useReadingStatsStore = create<ReadingStatsState>((set, get) => ({
  ...
  // 获取年度报告聚合数据
  getAnnualReport: async (userId: string, year: number) => {
    if (isCloudMode()) {
      try {
        const result = await apiGet<AnnualReportData>(`/api/reading-stats/annual?year=${year}`);
        return result;
      } catch {
        // 云端失败回退到本地
      }
    ...
    // 月度统计
    const monthlyStatsMap: Record<number, { duration: number; bookSet: Set<string> }> = {};
    for (let i = 1; i <= 12; i++) {
      monthlyStatsMap[i] = { duration: 0, bookSet: new Set() };
    }
    sessions.forEach((s) => {
      ...
      if (parts.length >= 2 && parts[1]) {
        ...
        if (monthlyStatsMap[month]) {
          monthlyStatsMap[month].duration += s.duration;
          monthlyStatsMap[month].bookSet.add(s.bookId);
        }
      }
    });
    const monthly = Object.entries(monthlyStatsMap).map(([m, data]) => ({
      month: parseInt(m, 10),
      duration: data.duration,
      bookCount: data.bookSet.size,
    }));

    // 书籍排行 TOP 5
    const bookDurations = new Map<string, AnnualReportData['topBooks'][0]>();
    sessions.forEach((s) => {
      const book = bookMap.get(s.bookId);
      if (!bookDurations.has(s.bookId)) {
        bookDurations.set(s.bookId, {
          bookId: s.bookId,
          title: book?.title || '未知书籍',
          author: book?.author || '',
          coverUrl: book?.coverUrl || '',
      ...
      const item = bookDurations.get(s.bookId)!;
      item.duration += s.duration;
    });
    const topBooks = Array.from(bookDurations.values())
      .sort((a, b) => b.duration - a.duration)
      .slice(0, 5);
    ...
    const formatDistribution = Object.entries(formatCounts).map(([format, count]) => ({
      format,
      count,
      percentage: totalFormatCount > 0 ? Math.round((count / totalFormatCount) * 100) : 0,
    }));
    ...
    const favoriteWeekdayEntry = Object.entries(weekdayCounts).sort((a, b) => b[1] - a[1])[0];
    const favoriteHourEntry = Object.entries(hourCounts).sort((a, b) => b[1] - a[1])[0];
    const favoriteWeekday = favoriteWeekdayEntry ? parseInt(favoriteWeekdayEntry[0], 10) : null;
    const favoriteHour = favoriteHourEntry ? parseInt(favoriteHourEntry[0], 10) : null;
