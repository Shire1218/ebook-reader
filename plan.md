...
### P6 - 网页优化（已完成）
- [x] 做一个网页展览页面，landing页面
- [x] 开发登录模块，支持jwt认证，隔离用户
- [x] 支持联网，跳转到外部网页下载书籍
...
### P-extra - 支持代码文件的编辑（待开发）
构想：将系统做成双模式，阅读模式，集成目前的所有功能；编辑模式，支持对文件进行编辑，甚至是运行代码文件，支持常见编程语言
- [ ] 标注 格式（支持基本语法）
- [ ] 代码高亮（支持多种编程语言）
- [ ] 代码块格式（缩进、对齐）
- [ ] 可以运行代码文件
- [ ] 系统打包，支持在浏览器中直接运行，同时支持在桌面端运行（详见 [桌面端迁移计划](plan/desktop_migration_plan.md)）
